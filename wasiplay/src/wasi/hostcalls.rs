//! TODO: in here is where we call our own functions, that then might call:
//!     - POSIX stubs over `wasi_common::hostcalls::*`
//!         when replicating read/written data
//!     - `datalove_wasi::hostcall_impl`
//!         when
//!     - `wasi_common::hostcalls::*` directly
//!         ... when getting printing to stdout...?
...
pub unsafe fn __wasi_sock_shutdown(
    ...
    let wasi_ctx = &lucet_ctx.get_embed_ctx::<WasiCtx>();
    let heap = &mut lucet_ctx.heap_mut();
    sock_shutdown(wasi_ctx, heap, sock, how)
}

pub fn export_wasi_funcs() {
    println!("exporting hostcalls");
