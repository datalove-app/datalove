...
impl<'a> Operation<'a, SetExchangeRateError> for SetExchangeRateOperation {
	fn ledger_id(&self) -> &Hash { &self.ledger_id }

	fn validate(
		&self,
		ledger_state: &LedgerState,
	) -> Result<&Self, SetExchangeRateError> {
		if self.is_rate_malformed() {
			Err(SetExchangeRateError::MalformedExchangeRateError)
		} else if self.is_ledger_capacity_exceeded(&ledger_state.ledger()) {
			Err(SetExchangeRateError::ExceededLedgerCapacityError)
		} else if self.is_ledger_underfunded(&ledger_state.ledger()) {
			Err(SetExchangeRateError::UnderfundedLedgerError)
		} else {
			Ok(self)
		}
	}
