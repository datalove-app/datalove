...
///
pub struct Encoder<W: io::Write>(JsonSerializer<W>);
...
where
    W: io::Write,
{
    /// Creates a new `Encoder` from an `io::Write`.
...
where
    W: io::Write,
{
    /// Serialize bytes as `{"/": { <multibase_code_string>: <<multibase_string>> }}`.
    #[inline]
...
/// Implement `Serializer` by delegating to the raw `serde_json::Serializer`, overriding `serialize_bytes`, and re-writing the recursive `serialize_*` methods to recurse with an `Encoder`.
...
mod tests {
    ...
    fn test_struct() {
        ...
        let expected = format!(
            r#"{{"some_bytes":{bytes},"aa_link":{cid}}}"#,
