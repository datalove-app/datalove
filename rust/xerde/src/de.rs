...
impl<'de, 'a: 'de> de::Deserializer<'de> for Deserializer<'a> {
    ...
    // TODO: assert that __struct__ is same as `name`
        ...
        V: Visitor<'de>,
    {
        ...
        let struct_name = self.term.map_get(__struct__).map_err(|_| Error::ExpectedStruct)?;
        ...
        V: Visitor<'de>,
    {
        match self.term.get_type() {
            TermType::Atom => match atoms::nil().eq(&self.term) {
                true => visitor.visit_unit(),
                false => {
                    // TODO: fix this to align with serializer
                    let atom_name = self.term.atom_to_string().map_err(|_| Error::ExpectedAtom)?;
