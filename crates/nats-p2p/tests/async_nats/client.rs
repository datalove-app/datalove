...
async fn queue_sub() {
    ...
    for mut subscriber in subscribers.into_iter() {
        results.push(tokio::spawn(async move {
            let mut count = 0u32;
            while let Ok(Some(_)) =
                tokio::time::timeout(tokio::time::Duration::from_millis(1000), subscriber.next())
                    .await
            {
                count += 1;
            }
...
async fn connection_callbacks() {
    ...
    let _server = nats_p2p::run_server_with_port("", Some(port.as_str()));
...
async fn publish_error_should_be_nameable() {
    let server = nats_p2p::run_basic_server();
    let client = async_nats::connect(server.client_url()).await.unwrap();
    let _error: Result<(), async_nats::PublishError> = client.publish("foo", "data".into()).await;
}
...
async fn retained_servers_order() {
    let mut servers = vec![nats_p2p::run_basic_server(), nats_p2p::run_basic_server()];
    let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
