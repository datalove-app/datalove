...
impl<'a> MiMCDemo<'a> {
    pub fn new<L: Into<Fr>, R: Into<Fr>>(xl: L, xr: R, constants: &'a [Fr]) -> Self {
        Self {
            xl: Some(xl.into()),
            xr: Some(xr.into()),
            constants,
...
impl<'a> From<&'a [Fr]> for MiMCDemo<'a> {
    fn from(constants: &'a [Fr]) -> Self {
        MiMCDemo {
            xl: None,
            xr: None,
            constants,
...
impl<'a> ConstraintSynthesizer<Fr> for MiMCDemo<'a> {
...
// pub fn gen_params<R: Rng>(circuit: ConstraintSynthesizer<Fr>, &mut rng: R) ->
