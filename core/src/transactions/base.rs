...
pub type LedgerIds<'a> = HashSet<OperationLedgerId<'a>>;
pub type Operations<'a> = Vec<LedgerOperation<'a>>;
pub type SequenceNumbers<'a> = HashMap<OperationLedgerId<'a>, u64>;
...
pub trait Transaction<'a, TxError: Error> {
    ...
    fn seq_ledger_ids(&self) -> LedgerIds<'a> {
        self.seq_nos()
            .keys()
            .fold(HashSet::new(), |mut ids, ref id| {
                ids.insert(id);
                ids
            })
    }
