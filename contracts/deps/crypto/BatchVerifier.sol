...
library BatchVerifier {
    uint256 public constant BaseField = 21888242871839275222246405745257275088696311157297823662689037894645226208583;
    uint256 public constant ScalarField = 21888242871839275222246405745257275088548364400416034343698204186575808495617;

    function NegateY(uint256 Y) internal pure returns (uint256) {
        return BaseField - (Y % BaseField);
    }
    ...
    function accumulate(
        ...
        for (uint256 proofNumber = 0; proofNumber < num_proofs; proofNumber++) {
            ...
            inputAccumulators[0] = addmod(inputAccumulators[0], entropy[proofNumber], ScalarField);
            for (uint256 i = 0; i < numPublicInputs; i++) {
                require(proof_inputs[proofNumber * numPublicInputs + i] < ScalarField, "INVALID_INPUT");
                // accumulate the exponent with extra entropy mod q
                inputAccumulators[i+1] = addmod(inputAccumulators[i+1], mulmod(entropy[proofNumber], proof_inputs[proofNumber * numPublicInputs + i], ScalarField), ScalarField);
            }
        ...
        for (uint256 proofNumber = 1; proofNumber < num_proofs; proofNumber++) {
            require(entropy[proofNumber] < ScalarField, "INVALID_INPUT");
