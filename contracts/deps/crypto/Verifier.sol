...
library Verifier
{
    uint256 constant BaseField = 21888242871839275222246405745257275088696311157297823662689037894645226208583;
    ...
    */
    ...
    function Verify(
        ...
        for (uint i = 0; i < proof_inputs.length; i++) {
            require(proof_inputs[i] < ScalarField, "INVALID_INPUT");
            ...
            assembly {
                ...
                success := staticcall(sub(gas(), 2000), 0x07, mul_input, 0x80, add(add_input, 0x40), 0x60)
            }
            if (!success) {
                return false;
            }
            ...
            assembly {
                // ECADD
                success := staticcall(sub(gas(), 2000), 0x06, add_input, 0xc0, add_input, 0x60)
            }
            if (!success) {
                return false;
            }
        ...
        uint[1] memory out;
        assembly {
            success := staticcall(sub(gas(), 2000), 0x08, input, 768, out, 0x20)
        }
