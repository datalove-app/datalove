...
/-!
# Codegen Overrides - HVM overrides of Lean declarations
...
structure Override.Decl where
  declName : Lean.Name
  decl : Term -- ? should be Rule?

inductive Override.Alt where
  | alt (cidx : Nat) (params : Array Lean.Name) (code : Term)
  | default (code : Term)

instance : ToString Override.Alt where toString
...
structure Override.Inductive where
  ...
  mkCases : (discr : Term) → (alts : Array Override.Alt) → Except String Term
