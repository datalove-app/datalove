...
namespace HVM.Overrides

open HVM Codegen DSL Datatypes Term

def NatInductiveData : InductiveData := ⟨``Nat, 0, 0,
  .ofList [(``Nat.zero, 0), (``Nat.succ, 1)]
⟩
...
def NatMkCases (discr : Term) (alts : Array Override.Alt) : Except String Term := do
  dbg_trace ">> NatMkCases: {discr} {alts}"
  ...
  -- let cases := Term.mkIfElses ifThens.toList defaultElse
  return nil

-- FIXME: yatima's has a Fin
def NatCore : Override.Decl := ⟨``Nat, ⟦nil⟧⟩
